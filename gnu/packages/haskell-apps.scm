  ...
  (package
    ...
    (source
     (origin
       ...
       (patches (search-patches "stack-ghc8.8-compatibility.patch"))))
    (build-system haskell-build-system)
    ;; FIXME: Some tests fail to access root directory
    (arguments `(#:tests? #f))
    (inputs
     `(("ghc-aeson" ,ghc-aeson)
...
(define-public threadscope
  (package
    ...
    (source
     (origin
       (method url-fetch)
       (uri (hackage-uri "threadscope" version))
       (sha256
        (base32 "0hfn90299mp18p2bjj2bgfi1pk3ndxz4nv2ac6z52kvh00sr9drq"))))
    (build-system haskell-build-system)
    (properties '((upstream-name . "threadscope")))
    (inputs (list ghc-gtk
                  ghc-cairo
                  ghc-glib
                  ghc-pango
    ...
    (description
     "@code{ThreadScope} is a graphical viewer for thread profile information
generated by the Glasgow Haskell compiler (GHC). .  The Threadscope program
allows us to debug the parallel performance of Haskell programs.  Using
Threadscope we can check to see that work is well balanced across the available
processors and spot performance issues relating to garbage collection or poor
load balancing.")
    (license license:bsd-3)))
